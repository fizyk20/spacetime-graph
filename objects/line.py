...
class Line(GenericObject):
    ...
    def __init__(self, point=(0,0,0), dir=(1,0,0), **kwargs):
        super(Line, self).__init__(**kwargs)
        
        if not isinstance(point, tuple) or len(point) != 3:
            raise Exception
        
        if not isinstance(dir, tuple) or len(dir) != 3:
            raise Exception
    ...
    def _draw(self, **kwargs):
        p1 = numpy.array(kwargs.get('p1', (-1, -1, -1)), 'f')
        p2 = numpy.array(kwargs.get('p2', (1, 1, 1)), 'f')
        pl = [p1, p2]
        t1 = 0
        t2 = 0
        for i in range(8):
        ...
        glBegin(GL_LINES)
        glColor(*self._color)
        glVertex(p1[0], p1[1], p1[2])
        glVertex(p2[0], p2[1], p2[2])
        glEnd()